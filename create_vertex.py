...
class vertex_window(QDialog):
    def __init__(self, coordinate_names):
        ...
        for coordinate in coordinate_names:
            self.checkboxs.append(QCheckBox(coordinate[0]))
            self.checkboxs.append(QCheckBox(coordinate[1]))
    ...
    def read_box(self):
        for i in range(0, len(self.checkboxs),2):
            value = [int(self.checkboxs[i].isChecked()), int(self.checkboxs[i+1].isChecked())]
